...
const saveSettingsToClipboard = () => {
    ...
    // Add model transform if model is loaded (use global model reference) hello!
    if (window.model) {
        settings.model = {
...
async function fetchWPSettingsAndApply() {
    try {
        ...
        // --- Apply materialImage as a texture to a target mesh ---
        // Replace 'Bumper_Plate001_COLOR_1001_0' with your actual mesh name if needed
        if (window.model && wpSettings.materialImage) {
            console.log('Material image URL from WP:', wpSettings.materialImage);
            window.model.traverse((child) => {
                if (child.isMesh && child.name === 'Bumper_Plate001_COLOR_1001_0') {
                    const loader = new THREE.TextureLoader();
                    loader.load(wpSettings.materialImage, function(texture) {
...
// --- Apply WP color to Bumper_Plate001_COLOR_1001_0 if premium ---
// Remove color update for Bumper_Plate001_COLOR_1001_0; handled by texture from WP
function applyBumperPlateMaterialImage(child) {
    if (child.name === 'Bumper_Plate001_COLOR_1001_0' && wpSettings.materialImage) {
        const loader = new THREE.TextureLoader();
        loader.load(wpSettings.materialImage, function(texture) {
            child.material.map = texture;
            child.material.needsUpdate = true;
        ...
        (gltf) => {
            ...
            model.traverse((child) => {
                if (child.isMesh) {
                    ...
                    child.material.needsUpdate = true;
                    // Apply color to bumper plate if premium
                    applyBumperPlateColorIfPremium(child);
