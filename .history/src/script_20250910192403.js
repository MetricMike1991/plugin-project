...
// UPDATED: centralized texture applier for multiple mesh names
function applyTextureToMultipleMeshes(textureUrl, meshNames) {
    if (!window.model || !textureUrl || !Array.isArray(meshNames)) return;
        ...
        (texture) => {
            ...
            window.model.traverse((child) => {
                if (child.isMesh && meshNames.includes(child.name)) {
                    ...
                    appliedMeshes.push(child.name);
                }
            });
            if (appliedMeshes.length === 0) {
                console.warn(`[WP3D] None of the target meshes found to apply texture.`);
            } else {
                console.log(`[WP3D] Applied texture to meshes:`, appliedMeshes);
            }
        },
        undefined,
...
async function fetchWPSettingsAndApply() {
    try {
        ...
        // UPDATED: if we already have the model, apply to all target meshes
        if (wpSettings.materialImage) {
            const targetMeshNames = [
                'Bumper_PlateLeft_COLOR_1_0',
                'Bumper_Plate_Left_COLOR_1_0',
                'Bumper_Plate002_COLOR_1_0',
                'Bumper_Plate003_COLOR_1_0'
            ];
            applyTextureToMultipleMeshes(wpSettings.materialImage, targetMeshNames);
        }
    } catch (err) {
...
function loadOverheadPressModel() {
    const modelPath = 'https://FlexFrame.b-cdn.net/02.%20Deadlift%20GLB%20Final.glb';
...
canvas.addEventListener('pointerdown', (event) => {
    ...
    if (intersects.length > 0) {
        ...
        updateMeshTestParams(mesh);
            // Track last clicked mesh for material logging
            window.lastClickedMesh = mesh;
    }
});

    // Listen for 'm' key to log material properties of last clicked mesh
    window.addEventListener('keydown', (event) => {
        if (event.key.toLowerCase() === 'm' && window.lastClickedMesh && window.lastClickedMesh.material) {
            console.log('Material properties:', window.lastClickedMesh.material);
        }
    });
